...
function loadVocabContent(lang, category) {
    ...
    // Pour la section français (pour russophones), titres en russe
    // Pour la section russe (pour francophones), titres en français
    const categoryNames = {
        salutations: lang === 'fr' ? 'Приветствия' : 'Les Salutations',
        nombres: lang === 'fr' ? 'Числа' : 'Les Nombres',
        famille: lang === 'fr' ? 'Семья' : 'La Famille',
        couleurs: lang === 'fr' ? 'Цвета' : 'Les Couleurs',
        animaux: lang === 'fr' ? 'Животные' : 'Les Animaux',
        nourriture: lang === 'fr' ? 'Еда' : 'La Nourriture',
        corps: lang === 'fr' ? 'Тело' : 'Le Corps',
        temps: lang === 'fr' ? 'Время' : 'Le Temps',
        transports: lang === 'fr' ? 'Транспорт' : 'Les Transports',
        maison: lang === 'fr' ? 'Дом' : 'La Maison',
        vetements: lang === 'fr' ? 'Одежда' : 'Les Vêtements',
        meteo: lang === 'fr' ? 'Погода' : 'La Météo',
        actions: lang === 'fr' ? 'Действия' : 'Les Actions',
        pays: lang === 'fr' ? 'Страны' : 'Les Pays'
    };
...
function loadPhrasesContent(lang, category) {
    ...
    // Pour la section russe (pour francophones), titres en français
    const categoryNames = {
        restaurant: lang === 'fr' ? 'В Ресторане' : 'Au Restaurant',
        hotel: lang === 'fr' ? 'В Отеле' : 'À l\'Hôtel',
        magasin: lang === 'fr' ? 'В Магазине' : 'Au Magasin',
        voyage: lang === 'fr' ? 'В Путешествии' : 'En Voyage',
        urgence: lang === 'fr' ? 'Срочность' : 'Urgences'
    };
...
function loadExercise(lang) {
    ...
    if (currentExerciseType[lang] === 'choix' && exercise.alternatives && exercise.alternatives.length > 0) {
        ...
        // Pour la section français (pour russophones), placeholder en russe
        // Pour la section russe (pour francophones), placeholder en français
        contentEl.innerHTML = `
            <input type="text" id="answer-${lang}" placeholder="${lang === 'fr' ? 'Ваш ответ на русском...' : 'Votre réponse en français...'}">
            <button class="btn-check" onclick="checkExerciseAnswer('${lang}')">${lang === 'fr' ? 'Проверить' : 'Vérifier'}</button>
        `;
    }
...
function checkExerciseAnswer(lang) {
    ...
    if (isCorrect) {
        ...
        // Pour la section français (pour russophones), messages en russe
        // Pour la section russe (pour francophones), messages en français
        resultEl.textContent = lang === 'fr' ? '✓ Правильно! Отличный ответ!' : '✓ Correct ! Excellente réponse !';
        ...
        resultEl.textContent = `${lang === 'fr' ? '✗ Неправильно. Правильный ответ:' : '✗ Incorrect. La bonne réponse est :'} "${exercise.reponse}"`;
...
function checkChoiceAnswer(lang, selected, correct) {
    ...
    if (isCorrect) {
        ...
        // Pour la section français (pour russophones), messages en russe
        // Pour la section russe (pour francophones), messages en français
        resultEl.textContent = lang === 'fr' ? '✓ Правильно!' : '✓ Correct !';
        resultEl.className = 'result success';
    } else {
        stats.incorrect++;
        stats.currentStreak = 0;
        resultEl.textContent = `${lang === 'fr' ? '✗ Неправильно.' : '✗ Incorrect.'} ${lang === 'fr' ? 'Правильный ответ:' : 'La bonne réponse est :'} "${correct}"`;
        resultEl.className = 'result error';
    }
...
function showFlashcard(lang, words) {
    ...
    // Pour la section français (pour russophones), consignes en russe
    // Pour la section russe (pour francophones), consignes en français
    questionEl.textContent = lang === 'fr' ? `Переведите : "${word.fr}"` : `Traduisez : "${word.ru}"`;
    contentEl.innerHTML = `
        <div class="flashcard">
            <div class="flashcard-front">
                <p class="flashcard-word">${lang === 'fr' ? word.fr : word.ru}</p>
                <button class="btn-flip" onclick="flipFlashcard('${lang}')">${lang === 'fr' ? 'Показать ответ' : 'Voir la réponse'}</button>
            </div>
            <div class="flashcard-back" style="display:none;">
                <p class="flashcard-word">${lang === 'fr' ? word.ru : word.fr}</p>
                <button class="btn-flip" onclick="flipFlashcard('${lang}')">${lang === 'fr' ? 'Скрыть' : 'Masquer'}</button>
            </div>
        </div>
    `;
    resultEl.textContent = '';
